...
- Impact: **Every COG-path reflectance for a post-2022-01-25 scene read 0.1 too high.** Because an additive offset does not cancel in `(a-b)/(a+b)`, this biased *all* normalized ratios toward zero, not only the 12 live Atlas records using absolute reflectance thresholds. Worked case: NDVI from DN 3000/1500 returns 0.33 uncorrected vs 0.60 corrected — a 0.27 error. COG is the configured **default** provider for map rendering.
- **Blast radius, checked 2026-07-25 (an earlier draft of this entry overstated it):** the produced-water threshold chain is NOT affected. `fetch_spill_bands.py` and `sample_background.py` pull band means through `batch_analyze_spills.fetch_stats`, which calls the **Sentinel Hub Statistics API** — a harmonized path. So `validation_raw.csv`, `background_raw.csv` (150 controls), `spill_bands.csv`, the 1,224-combination sweep, and `best_thresholds.json` were all computed on correctly offset reflectance despite every one of their dates (2022-02-14 to 2024-01-22) falling after the baseline change. They do not need refitting for this reason. What IS affected is the 2026-07-23 visual QC pass, which rendered the 13 spill bookmarks at 2025 dates through `render_cog_tile.py`; those candidate-strength percentages (e.g. OBEC 10.2% at Matador Desoto) were computed on reflectance 0.1 high and should be re-run.
