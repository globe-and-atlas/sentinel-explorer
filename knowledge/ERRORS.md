...
- Impact: **Every COG-path reflectance for a post-2022-01-25 scene read 0.1 too high.** Because an additive offset does not cancel in `(a-b)/(a+b)`, this biased *all* normalized ratios toward zero, not only the 12 live Atlas records using absolute reflectance thresholds. Worked case: NDVI from DN 3000/1500 returns 0.33 uncorrected vs 0.60 corrected — a 0.27 error. COG is the configured **default** provider, and the produced-water spill dates (2023–2025) are entirely post-baseline, so `best_thresholds.json` was tuned on biased inputs.
