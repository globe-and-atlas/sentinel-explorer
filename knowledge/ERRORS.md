...
## 2026-07-25 — COG provider omitted the Sentinel-2 baseline 04.00 radiometric offset

- Error: `execution/render_cog_tile.py` converted Earth Search `sentinel-2-l2a` digital numbers to reflectance with a bare `DN / 10000.0`. Since ESA processing baseline 04.00 (operational 2022-01-25), L2A carries `BOA_ADD_OFFSET = -1000`, so correct reflectance is `(DN - 1000) / 10000`. Earth Search serves unharmonized ESA DN.
- Cause: The conversion was written before baseline 04.00 mattered and was never revisited. Nothing in `execution/` referenced the offset, the baseline, or harmonization. The bug was invisible because it produces plausible-looking imagery — every value is simply shifted.
- Impact (**revised 2026-07-25 after checking real STAC metadata — the first two drafts of this entry overstated it twice**): Earth Search exposes `earthsearch:boa_offset_applied`, and for most items it is `true`, meaning the archive already spent the offset and `DN / 10000` was correct. The defect is the **subset where it is `false` on a baseline >= 04.00 item**: those read **0.1 reflectance too high** while neighbouring scenes read correctly. Measured over the Permian test bbox: 38 of 589 items (6.5%), concentrated in 2022 and 2025, none in 2023-2024. Because an additive offset does not cancel in `(a-b)/(a+b)`, this biased *all* normalized ratios toward zero, not only the 12 live Atlas records using absolute reflectance thresholds. Worked case: NDVI from DN 3000/1500 returns 0.33 uncorrected vs 0.60 corrected — a 0.27 error. COG is the configured **default** provider for map rendering. The scene-dependent nature is the real hazard: a uniform bias would at least be consistent, whereas this makes two dates of the same site differ by 0.1 with nothing in the output to show it, so any multi-date comparison built on the raw expression is silently inconsistent.
- **Blast radius, checked 2026-07-25 (earlier drafts of this entry overstated it twice):** no recorded project result is affected. The produced-water threshold chain is not affected. `fetch_spill_bands.py` and `sample_background.py` pull band means through `batch_analyze_spills.fetch_stats`, which calls the **Sentinel Hub Statistics API** — a harmonized path. So `validation_raw.csv`, `background_raw.csv` (150 controls), `spill_bands.csv`, the 1,224-combination sweep, and `best_thresholds.json` were all computed on correctly offset reflectance despite every one of their dates (2022-02-14 to 2024-01-22) falling after the baseline change. They do not need refitting for this reason. The 2026-07-23 COG visual QC pass is also not affected: re-running it via the new `execution/qc_spill_cog_render.py` resolved `boa_offset = 0.0` for **all 56 tiles** — every scene those bookmarks pull already had the offset applied by Earth Search. The qualitative conclusion holds (Matador Desoto and Lake Boehmer strongest, most other sites near-blank), though absolute percentages differ from the ad-hoc pass (OBEC at Matador Desoto 6.086% vs 10.2%). **That delta is parameter-driven, not offset-driven** — the new script fixes zoom/tile/window/maxcc explicitly where the 2026-07-23 pass did not record them — so do not attribute it to the fix.
- **Net:** the defect is real and latent (it would silently corrupt any future COG-rendered multi-date work) but it never reached a published or recorded number. It was caught before it did.
- Not affected: the Sentinel Hub WMS path (`harmonizeValues` defaults to true — verified against Sentinel Hub docs) and the GEE path (`COPERNICUS/S2_SR_HARMONIZED` is pre-harmonized). The 2026-07-21 Atlas display audit used WMS, so its "nonblank overlay for all 37" result stands.
- Fix: `resolve_boa_offset()` resolves the offset per scene, preferring `earthsearch:boa_offset_applied` (guards against double-correction when the archive already applied it), then `s2:processing_baseline`, then acquisition date vs the 2022-01-25 cutover. `item_boa_offset()` handles items cached before the fix, which carry neither new property. `ITEM_CACHE_SCHEMA` bumped so stale cache entries are not reused. A clarifying comment was added at the GEE `.divide(10000)` site so nobody "fixes" it by analogy.
- Verification: `tests/test_cog_boa_offset.py` — baseline thresholds, the double-correction guard, the legacy-cache fallback, and the numeric consequence. Run with `.venv/bin/python3` (rasterio is not in the system interpreter).
- Graduated to: `knowledge/domain/api-contracts.md` (provider reflectance conventions) and the preprint erratum at `remote-sensing-research/preprint/gsia_preprint_v2_erratum_2026-07-25.md`.
